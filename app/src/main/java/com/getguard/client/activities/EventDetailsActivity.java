...
public class EventDetailsActivity extends AppCompatActivity {
    ...
    private int weapon = 0, personalCar = 0;
    ...
    private void configureView() {
        switch (viewState) {
            ...
            case unregister:
                disableViewEditing();
                publishLayout.setVisibility(View.VISIBLE);
                publishText.setText("Отозвать отклик");
                publishLayout.setOnClickListener(v -> {
                    unrespond();
                });
                getEvent();
    ...
    private void enableViewEditing() {
        ...
        startTimeInput.setOnClickListener(v -> {
            new SingleDateAndTimePickerDialog.Builder(EventDetailsActivity.this)
                    .bottomSheet()
                    .curved()
    ...
    private void unrespond() {
        ...
        NetworkManager.getInstance(this).unrespond(user.getToken(), id, (errorMessage, data) -> {
            progressDialog.dismiss();
